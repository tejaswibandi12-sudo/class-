...
def analyze_sentiment():
    text_input = entry.get()  # Get text from entry box
    ...
    result_label.config(text=f"Sentiment: {sentiment}", fg="blue", font=("Arial", 12, "bold"))

def open_sentiment_window():
    sentiment_window = Toplevel(main)
    sentiment_window.title("Sentiment Analysis")
    sentiment_window.geometry("400x250")
    
    # Title Label
    title_label = Label(sentiment_window, text="Sentiment Analysis", font=("Arial", 16, "bold"))
    title_label.pack(pady=10)
    
    # Text Entry
    global entry
    entry = Entry(sentiment_window, width=50, font=("Arial", 12))
    entry.pack(pady=10)
    
    # Submit Button
    submit_button = Button(sentiment_window, text="Analyze", command=analyze_sentiment, font=("Arial", 12, "bold"), bg="#007bff", fg="white", padx=10, pady=5)
    submit_button.pack(pady=5)
    
    # Result Label
    global result_label
    result_label = Label(sentiment_window, text="", font=("Arial", 12), fg="black")
...
# Add button for Sentiment Analysis
sentimentButton = Button(main, text="Sentiment Analysis", command=open_sentiment_window)
sentimentButton.place(x=1050, y=150)
